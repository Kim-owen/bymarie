...
async function sendOrderStatusNotification(order, status) {
  ...
  const clientName = order.name || 'Valued Client';
  const customerPhoneFormatted = formatPhoneForGhana(order.phone);
  ...
  // 1. Dispatch SMS Update
  if (mnotifyKey && customerPhoneFormatted && customerPhoneFormatted.length >= 10) {
    let smsMessage = `ByMarie Update: Order #${orderId} status is now "${status}".`;
    if (status === 'Dispatched' || status === 'Out for Delivery') {
      smsMessage = `ByMarie Alert: Hello ${clientName}, Order #${orderId} has been dispatched! Your package is en route to ${order.city || 'your address'}. Track: https://bymarie.shop/#account`;
    } else if (status === 'Delivered' || status === 'Completed') {
      smsMessage = `ByMarie: Order #${orderId} has been successfully delivered! Thank you for shopping with ByMarie. Enjoy your luxury piece! ✨`;
    }
    ...
    try {
      ...
      console.log(`📱 [ORDER STATUS SMS] Sent to ${clientName} (${customerPhoneFormatted}) for Order #${orderId}: ${status}`);
    } catch (e) {
      console.warn('Order status SMS error:', e.message);
    }
  }

  // 2. Dispatch Email Update via Resend
  const resendApiKey = config.RESEND_API_KEY;
  const fromAddress = config.RESEND_FROM_EMAIL || 'ByMarie Concierge <concierge@bymarie.shop>';
  ...
  if (resendApiKey && customerEmail && customerEmail.includes('@')) {
    try {
      ...
      const emailHtml = `
        <div style="font-family:'Montserrat',Helvetica,Arial,sans-serif;max-width:600px;margin:0 auto;background:#09090b;color:#fff;padding:40px 24px;border-radius:12px">
          <div style="text-align:center;margin-bottom:24px">
            <span style="font-size:11px;letter-spacing:3px;color:#c24d67;text-transform:uppercase;font-weight:700">ORDER STATUS UPDATE</span>
            <h1 style="font-size:24px;font-weight:900;margin:8px 0 0;color:#fff">Order #${orderId}</h1>
          </div>

          <div style="background:#18181b;border:1px solid #27272a;padding:24px;border-radius:10px;margin-bottom:20px">
            <p style="color:#d4d4d8;font-size:15px;margin:0 0 16px">Dear ${clientName},</p>
            <p style="color:#a1a1aa;font-size:14px;line-height:1.6;margin:0 0 16px">
              Your order status has been updated to: <strong style="color:#facc15;font-size:16px">${status.toUpperCase()}</strong>
            </p>
            <div style="background:#27272a;padding:16px;border-radius:6px;margin-bottom:16px">
              <span style="display:block;font-size:12px;color:#a1a1aa">Delivery Address:</span>
              <strong style="color:#fff;font-size:14px">${order.address || ''}, ${order.city || ''}</strong>
            </div>
            <a href="https://bymarie.shop/#account" style="background:#c24d67;color:#fff;text-decoration:none;padding:12px 24px;border-radius:6px;font-weight:700;font-size:13px;display:inline-block">View Order Status →</a>
          </div>

          <p style="text-align:center;color:#71717a;font-size:11px;margin:0">ByMarie Luxury Atelier • Ghana</p>
        </div>
      `;
      ...
      await resend.emails.send({
        from: fromAddress,
        to: customerEmail,
        subject: `⚡ Order Status Update #${orderId}: ${status} - ByMarie`,
        html: emailHtml
      });
      console.log(`📧 [ORDER STATUS EMAIL] Sent to ${customerEmail} for Order #${orderId}: ${status}`);
    } catch (e) {
      console.warn('Order status email error:', e.message);
    }
  }
}

module.exports = { sendAdminOrderNotifications, notifyAllUsersNewProduct, sendOrderStatusNotification };
